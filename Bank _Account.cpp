...
openAccount(Account user){
    ...
    user.setBalance(0);             //intially 0 balance
    
    ofstream outfile("D:/Account.txt", ios::app);      //writing in file
    if(!outfile){
...
addCash(){
    ...
    ifstream infile("D:/Account.txt");      //read from file
...
int main(){
    ...
    while(!exit){
        system("cls");          //clear output of prev interation
        int val;
        ...
        Sleep(3000);            //in ms, for how much time we want to display o/p
    }
}
